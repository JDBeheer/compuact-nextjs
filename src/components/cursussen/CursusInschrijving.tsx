...
function OlcInschrijving({ prijs, cursusTitel, addToCart, items }: {
  prijs: number
  cursusTitel: string
  addToCart: (item: CartItem) => void
  items: CartItem[]
}) {
  const olcId = `olc-${cursusTitel}`
  const inCart = items.some(i => i.sessieId === olcId)
  ...
  return (
    <div className="mb-6">
      ...
      <div className={cn(
        ...
        <div className="bg-gradient-to-r from-zinc-600 via-zinc-700 to-zinc-800 p-5 lg:p-6 text-white relative">
          <div className="absolute inset-0 opacity-[0.06]" style={{ backgroundImage: 'radial-gradient(circle, white 1px, transparent 1px)', backgroundSize: '20px 20px' }} />
          <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-5">
            <div className="flex-1">
              <div className="flex items-center gap-2.5 mb-2">
                ...
                <div>
                  <h3 className="font-bold text-lg">Open Leercentrum (OLC)</h3>
                  <p className="text-zinc-300 text-sm">Zelfstandig leren met begeleiding · Den Bosch</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-3 text-sm text-zinc-300">
                <span className="flex items-center gap-1.5"><CheckCircle size={13} /> Werk in je eigen tempo</span>
                <span className="flex items-center gap-1.5"><CheckCircle size={13} /> Begeleiding van een docent</span>
                <span className="flex items-center gap-1.5"><CheckCircle size={13} /> Lesmateriaal inbegrepen</span>
              </div>
            </div>
            <div className="flex items-center gap-4 shrink-0">
              <div className="text-right">
                <div className="text-2xl font-extrabold">{formatPrice(prijs)}</div>
                <div className="text-xs text-zinc-400">excl. BTW</div>
              </div>
              {inCart ? (
                <div className="w-12 h-12 rounded-xl bg-white text-zinc-700 flex items-center justify-center shadow-lg">
                  <Check size={22} strokeWidth={3} />
                </div>
              ) : (
                <button
                  onClick={handleAdd}
                  className="flex items-center gap-2 bg-white text-zinc-700 px-5 py-3 rounded-xl font-bold hover:bg-zinc-50 hover:shadow-lg transition-all active:scale-[0.98]"
                >
                  Toevoegen
                  <ArrowRight size={16} />
                </button>
